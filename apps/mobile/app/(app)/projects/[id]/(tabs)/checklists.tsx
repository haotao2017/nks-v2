...
function ChecklistCard({
  ...
  const openChecklist = () => {
    if (!projectId || !checklist.checklistId) return;
    router.push({
      pathname: '/(app)/projects/[id]/checklist/[checklistId]',
      params: {
        id: projectId,
  ...
  return (
    <Pressable
      ...
      className="rounded-2xl border border-neutral-200 bg-white p-4 active:bg-neutral-100 dark:border-neutral-800 dark:bg-neutral-900 dark:active:bg-neutral-800"
    >
...
export default function ChecklistsTab() {
  const routeId = useProjectRouteId();
  const { detail, status, error, projectId, reload } =
    useLoadActiveProject(routeId);
  const resolvedProjectId = projectId ?? detail?.projectID ?? routeId ?? '';
  ...
  if (detail.checklists.length === 0) {
    return (
      <ScreenEmpty
        ...
        hint="Opprett sjekklister for prosjektet i admin."
      />
    );
  }
  ...
  return (
    <FlatList
      ...
      renderItem={({ item }) => (
        <ChecklistCard projectId={resolvedProjectId} checklist={item} />
      )}
